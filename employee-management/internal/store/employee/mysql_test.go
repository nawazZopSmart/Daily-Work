...
func TestGetDetailsById(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			desc:        "success",
			id:          1,
			emp:         &e.Employee{1, "John", "jhn@yahoo.com", "intern"},
			expectedErr: nil,
		...
		},
		{
			desc:        "success",
			id:          2,
			emp:         &e.Employee{2, "Jane", "jane@yahoo.com", "intern"},
			expectedErr: nil,
	...
	for i, testCase := range testCases {
		...
		t.Run(testCase.desc, func(t *testing.T) {
			emp, err := handler.GetDetailsById(testCase.id)
...
func TestGetDeleteById(t *testing.T) {
	...
	for _, testCase := range testCases {
		err := handler.DeleteById(testCase.id)
		if err != nil && err.Error() != testCase.expectedErr.Error() {
			t.Errorf("expected error:%v, got:%v", testCase.expectedErr, err)
...
func TestUpdateById(t *testing.T) {
	...
	for i, testCase := range testCases {
		t.Run("", func(t *testing.T) {
			err := handler.UpdateById(testCases[i].id, testCases[i].name, testCases[i].email, testCases[i].role)
...
func TestInsertData(t *testing.T) {
	...
	for i, testCase := range testCases {
		t.Run("", func(t *testing.T) {
			err := handler.InsertData(testCases[i].name, testCases[i].email, testCases[i].role)
			if err != nil && err.Error() != testCase.expectError.Error() {
				t.Errorf("expected error:%v, got:%v", testCase.expectError, err)
			}
		})
